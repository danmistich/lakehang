...
## 4. (Optional) Host Access — see who picked what time slot

By default the results grid only shows counts (not names) per time block, so it scales to a big group without turning into a wall of text. "Host Access" lets specific people — you, co-hosts — sign in with Google to reveal names in every cell.

1. Firebase Console → **Build > Authentication > Get started**. Under **Sign-in method**, enable **Google**.
2. Still in Authentication, go to **Settings > Authorized domains** and add your GitHub Pages domain (e.g. `<your-username>.github.io`) — `localhost` is already there by default, which is why this works when testing locally before you've added anything.
...
4. Push. On the live site, click **Host Access** near the results grid — Google's sign-in popup appears, and if the signed-in email matches the allowlist, names appear in every cell for the rest of that browser session (persists across reloads until you sign out).

**Worth understanding — this is a real sign-in requirement, but not a data lockdown.** There's no password anywhere in the code to find or leak; only someone who can actually authenticate as an allowlisted Google account gets the reveal. That said, `rsvps` has to stay publicly readable (`allow read: if true` in `firestore.rules`) so the guest-facing headcount and overlap grid keep working with no login for anyone — which means the raw name-to-block data was already technically fetchable by a determined person poking at Firestore directly, same as before this feature existed. Host Access gates the convenient on-site *view*; it doesn't add a new lock to the underlying data. Making that airtight would require moving to a real backend (a Cloud Function computing the public aggregate separately from an auth-only raw collection), which is a lot more infrastructure than this feature is worth — Google Sheets (see step 3 above) is the better answer if you want that data genuinely private, since Google's own sharing permissions are real, server-enforced access control.
